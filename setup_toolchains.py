  ...
  for arch, buildroot_versions in _AVAILABLE_TOOLCHAINS.items():
    ...
    for version, info in buildroot_versions.items():
      ...
      for tool in _ALL_TOOLS:
        tool_name = f'{info["tool_prefix"]}-{tool}'
        wrapper_path = os.path.join(wrapper_dir, arch, version, tool_name)
        ...
        with open(wrapper_path, 'w') as f:
          ...
          f.write(f'exec external/{toolchain_name}/bin/{tool_name}.br_real $@')
